...
def mk_cost_lines(x,y,w,b, ax):
    ...
    for p in zip(x,y):
        ...
        c_p = ((f_wb_p - p[1])**2)/2
        c_p_txt = c_p
        ax.vlines(p[0], p[1],f_wb_p, lw=3, color=dlpurple, linestyle='dotted', label=label)
        label='' #just one
...
def plt_intuition(x_train, y_train):
    ...
    def func( w=150):
        ...
        ax[1].hlines(cur_cost, ax[1].get_xlim()[0],w, lw=4, color=dlpurple, linestyle='dotted')
        ax[1].vlines(w, ax[1].get_ylim()[0],cur_cost, lw=4, color=dlpurple, linestyle='dotted')
